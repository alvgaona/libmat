#define MATDEF static inline
#define MAT_IMPLEMENTATION
#include "mat.h"
#include "test.h"

static void test_bilinear_2x2(void) {
    TEST_BEGIN("bilinear_2x2");

    // x = [1, 2], A = [[1, 2], [3, 4]], y = [5, 6]
    // x^T * A * y = [1, 2] * [[1, 2], [3, 4]] * [5, 6]^T
    //             = [1, 2] * [1*5 + 2*6, 3*5 + 4*6]^T
    //             = [1, 2] * [17, 39]^T
    //             = 1*17 + 2*39 = 17 + 78 = 95
    Vec *x = mat_vnew({1.0f, 2.0f});
    Mat *A = mat_new(2, {{1.0f, 2.0f}, {3.0f, 4.0f}});
    Vec *y = mat_vnew({5.0f, 6.0f});

    mat_elem_t result = mat_bilinear(x, A, y);
    CHECK_FLOAT_EQ_TOL(result, 95.0f, 1e-5f);

    mat_free_mat(x);
    mat_free_mat(A);
    mat_free_mat(y);

    TEST_END();
}

static void test_bilinear_quadform_identity(void) {
    TEST_BEGIN("bilinear_quadform_identity");

    // Quadratic form x^T * A * x with identity using mat_bilinear
    // x^T * I * x = x^T * x = ||x||^2
    Vec *x = mat_vnew({1.0f, 2.0f, 3.0f});
    Mat *I = mat_reye(3);

    mat_elem_t result = mat_bilinear(x, I, x);
    mat_elem_t expected = 1.0f + 4.0f + 9.0f; // 14

    CHECK_FLOAT_EQ_TOL(result, expected, 1e-5f);

    mat_free_mat(x);
    mat_free_mat(I);

    TEST_END();
}

static void test_quadform_identity(void) {
    TEST_BEGIN("quadform_identity");

    // mat_quadform convenience function
    Vec *x = mat_vnew({1.0f, 2.0f, 3.0f});
    Mat *I = mat_reye(3);

    mat_elem_t result = mat_quadform(x, I);
    mat_elem_t expected = 1.0f + 4.0f + 9.0f; // 14

    CHECK_FLOAT_EQ_TOL(result, expected, 1e-5f);

    mat_free_mat(x);
    mat_free_mat(I);

    TEST_END();
}

static void test_quadform_symmetric(void) {
    TEST_BEGIN("quadform_symmetric");

    // x = [1, 2], A = [[2, 1], [1, 3]]
    // x^T * A * x = [1, 2] * [[2, 1], [1, 3]] * [1, 2]^T
    //             = [1, 2] * [2+2, 1+6]^T = [1, 2] * [4, 7]^T = 4 + 14 = 18
    Vec *x = mat_vnew({1.0f, 2.0f});
    Mat *A = mat_new(2, {{2.0f, 1.0f}, {1.0f, 3.0f}});

    mat_elem_t result = mat_quadform(x, A);
    CHECK_FLOAT_EQ_TOL(result, 18.0f, 1e-5f);

    mat_free_mat(x);
    mat_free_mat(A);

    TEST_END();
}

static void test_bilinear_nonsquare(void) {
    TEST_BEGIN("bilinear_3x4_nonsquare");

    // Non-square matrix (3x4)
    // x = [1, 2, 3], A = 3x4 matrix, y = [1, 2, 3, 4]
    Vec *x = mat_vnew({1.0f, 2.0f, 3.0f});
    Mat *A = mat_new(4, {
        {1.0f, 2.0f, 3.0f, 4.0f},
        {5.0f, 6.0f, 7.0f, 8.0f},
        {9.0f, 10.0f, 11.0f, 12.0f}
    });
    Vec *y = mat_vnew({1.0f, 2.0f, 3.0f, 4.0f});

    // Row 0 dot y = 1*1 + 2*2 + 3*3 + 4*4 = 1 + 4 + 9 + 16 = 30
    // Row 1 dot y = 5*1 + 6*2 + 7*3 + 8*4 = 5 + 12 + 21 + 32 = 70
    // Row 2 dot y = 9*1 + 10*2 + 11*3 + 12*4 = 9 + 20 + 33 + 48 = 110
    // result = 1*30 + 2*70 + 3*110 = 30 + 140 + 330 = 500
    mat_elem_t result = mat_bilinear(x, A, y);
    CHECK_FLOAT_EQ_TOL(result, 500.0f, 1e-5f);

    mat_free_mat(x);
    mat_free_mat(A);
    mat_free_mat(y);

    TEST_END();
}

static void test_bilinear_simd(void) {
    TEST_BEGIN("bilinear_256x256_simd");

    // Larger matrix to exercise SIMD
    size_t n = 256;
    Vec *x = mat_vec(n);
    Mat *A = mat_mat(n, n);
    Vec *y = mat_vec(n);

    // Fill with simple values
    for (size_t i = 0; i < n; i++) {
        x->data[i] = 1.0f;
        y->data[i] = 1.0f;
        for (size_t j = 0; j < n; j++) {
            A->data[i * n + j] = 1.0f;
        }
    }

    // x^T * A * y where all are 1s
    // Each row dot y = n (sum of n ones)
    // result = sum of n rows, each contributing n = n * n = n^2
    mat_elem_t result = mat_bilinear(x, A, y);
    mat_elem_t expected = (mat_elem_t)(n * n);

    CHECK_FLOAT_EQ_TOL(result, expected, 1e-2f);

    mat_free_mat(x);
    mat_free_mat(A);
    mat_free_mat(y);

    TEST_END();
}

static void test_bilinear_vs_gemv_dot(void) {
    TEST_BEGIN("bilinear_vs_gemv_dot");

    // Compare with manual gemv + dot
    size_t m = 64, n = 128;
    Vec *x = mat_vec(m);
    Mat *A = mat_mat(m, n);
    Vec *y = mat_vec(n);

    for (size_t i = 0; i < m; i++) x->data[i] = (mat_elem_t)(i + 1);
    for (size_t i = 0; i < n; i++) y->data[i] = (mat_elem_t)(i + 1);
    for (size_t i = 0; i < m * n; i++) A->data[i] = (mat_elem_t)((i % 10) + 1);

    // Manual: temp = A * y, result = x^T * temp
    Vec *temp = mat_vec(m);
    mat_gemv(temp, 1.0f, A, y, 0.0f);
    mat_elem_t expected = mat_dot(x, temp);

    mat_elem_t result = mat_bilinear(x, A, y);

    // Use relative tolerance
    CHECK(fabsf(result - expected) / fabsf(expected) < 1e-4f);

    mat_free_mat(x);
    mat_free_mat(A);
    mat_free_mat(y);
    mat_free_mat(temp);

    TEST_END();
}

int main(void) {
    test_bilinear_2x2();
    test_bilinear_quadform_identity();
    test_quadform_identity();
    test_quadform_symmetric();
    test_bilinear_nonsquare();
    test_bilinear_simd();
    test_bilinear_vs_gemv_dot();

    TEST_SUMMARY();
}
