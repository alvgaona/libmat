#include <stdio.h>
#include <stdlib.h>
#include <cblas.h>

// Force single-threaded before OpenBLAS initializes
__attribute__((constructor))
static void force_single_thread(void) {
  setenv("OPENBLAS_NUM_THREADS", "1", 1);
  setenv("OMP_NUM_THREADS", "1", 1);
  setenv("GOTO_NUM_THREADS", "1", 1);
}

#define BENCH_ROUNDS 20
#define BENCH_WARMUP 5
#define BENCH_IMPLEMENTATION
#include "bench.h"

#define MAT_IMPLEMENTATION
#include "mat.h"

#ifdef MAT_DOUBLE_PRECISION
  #define BLAS_SYR cblas_dsyr
  #define PRECISION_NAME "float64"
  #define BENCH_FILL bench_fill_random_d
#else
  #define BLAS_SYR cblas_ssyr
  #define PRECISION_NAME "float32"
  #define BENCH_FILL bench_fill_random_f
#endif

void bench_syr(size_t n, int iterations) {
  printf("\n--- %zux%zu ---\n", n, n);

  Mat *A = mat_mat(n, n);
  Mat *A_blas = mat_mat(n, n);
  Vec *x = mat_vec(n);

  BENCH_FILL(x->data, n);
  mat_fill(A, 0);
  mat_fill(A_blas, 0);

  mat_elem_t alpha = 1.0;

  for (int i = 0; i < BENCH_WARMUP; i++) {
    mat_syr(A, alpha, x, 'L');
    BLAS_SYR(CblasRowMajor, CblasLower, (int)n, alpha, x->data, 1, A_blas->data, (int)n);
  }

  double libmat_times[BENCH_ROUNDS], blas_times[BENCH_ROUNDS];

  for (int r = 0; r < BENCH_ROUNDS; r++) {
    mat_fill(A, 0);
    uint64_t start = bench_now();
    for (int i = 0; i < iterations; i++)
      mat_syr(A, alpha, x, 'L');
    uint64_t end = bench_now();
    libmat_times[r] = bench_ns(start, end) / iterations / 1000.0;

    mat_fill(A_blas, 0);
    start = bench_now();
    for (int i = 0; i < iterations; i++)
      BLAS_SYR(CblasRowMajor, CblasLower, (int)n, alpha, x->data, 1, A_blas->data, (int)n);
    end = bench_now();
    blas_times[r] = bench_ns(start, end) / iterations / 1000.0;
  }

  BenchStats ls = bench_stats(libmat_times, BENCH_ROUNDS);
  BenchStats bs = bench_stats(blas_times, BENCH_ROUNDS);

  // FLOPS for SYR: n^2 (symmetric, so n(n+1)/2 updates, each with mul+add)
  double flops = (double)n * n;
  double gflops_libmat = flops / (ls.avg * 1000.0);
  double gflops_blas = flops / (bs.avg * 1000.0);

  printf("libmat:   %8.2f ± %.2f us  (%.2fx vs BLAS)  %.1f GFLOPS\n",
         ls.avg, ls.std, bs.avg / ls.avg, gflops_libmat);
  printf("OpenBLAS: %8.2f ± %.2f us                   %.1f GFLOPS\n",
         bs.avg, bs.std, gflops_blas);

  mat_free_mat(A);
  mat_free_mat(A_blas);
  mat_free_mat(x);
}

int main() {
  srand(42);
  bench_init();

  bench_print_summary("libmat vs OpenBLAS: SYR");
  printf("Precision: %s\n", PRECISION_NAME);
  printf("A += alpha * x * x^T (symmetric rank-1 update)\n");
  printf("Rounds: %d, OpenBLAS threads: %d\n", BENCH_ROUNDS, openblas_get_num_threads());

  bench_syr(64, 5000);
  bench_syr(128, 2000);
  bench_syr(256, 500);
  bench_syr(512, 100);
  bench_syr(1024, 20);

  return 0;
}
