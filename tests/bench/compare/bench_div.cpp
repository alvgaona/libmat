/*
 * bench_div.cpp - Compare DIV: libmat vs Eigen
 *
 * C = A / B (element-wise division)
 *
 * Build:
 *   make bench-compare-div
 */
...
static void fill_random_nonzero(mat_elem_t* data, size_t n) {
    for (size_t i = 0; i < n; i++) {
        data[i] = (Scalar)rand() / RAND_MAX + 0.1f;  // Avoid division by zero
    }
}

// libmat: C = A / B
void bench_libmat(zap_bencher_t* b, void* param) {
    div_ctx_t* ctx = (div_ctx_t*)param;
    // Memory bandwidth: read 2n + write n
    zap_bencher_set_throughput_bytes(b, 3 * ctx->n * sizeof(Scalar));

    ZAP_ITER(b, {
        mat_div(ctx->C, ctx->A, ctx->B);
        zap_black_box(ctx->C->data);
    });
}

// Eigen: C = A.cwiseQuotient(B)
void bench_eigen(zap_bencher_t* b, void* param) {
    div_ctx_t* ctx = (div_ctx_t*)param;
    zap_bencher_set_throughput_bytes(b, 3 * ctx->n * sizeof(Scalar));

    ZAP_ITER(b, {
        *ctx->eC = ctx->eA->cwiseQuotient(*ctx->eB);
...
int main(int argc, char** argv) {
    ...
    zap_compare_group_t* g = zap_compare_group("div");
    ...
    for (size_t s = 0; s < num_sizes; s++) {
        ...
        fill_random_nonzero(A->data, n);
        fill_random_nonzero(B->data, n);
