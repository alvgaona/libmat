/*
 * bench_cos.cpp - Compare COS: libmat vs Eigen
 *
 * B = cos(A) (element-wise cosine)
 *
 * Build:
 *   make bench-compare-cos
 */
...
static void fill_random(mat_elem_t* data, size_t n) {
    for (size_t i = 0; i < n; i++) {
        data[i] = (Scalar)rand() / RAND_MAX * 6.28f;  // [0, 2*pi]
    }
}

// libmat: B = cos(A)
void bench_libmat(zap_bencher_t* b, void* param) {
    cos_ctx_t* ctx = (cos_ctx_t*)param;
...
int main(int argc, char** argv) {
    ...
    zap_compare_group_t* g = zap_compare_group("cos");
