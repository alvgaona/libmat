/*
 * bench_pow.cpp - Compare POW: libmat vs Eigen
 *
 * B = A^p (element-wise power)
 *
 * Build:
 *   make bench-compare-pow
 */
...
static void fill_random_positive(mat_elem_t* data, size_t n) {
    for (size_t i = 0; i < n; i++) {
        data[i] = (Scalar)rand() / RAND_MAX * 10.0f + 0.1f;  // [0.1, 10]
    }
}

// libmat: B = A^p
void bench_libmat(zap_bencher_t* b, void* param) {
    pow_ctx_t* ctx = (pow_ctx_t*)param;
    zap_bencher_set_throughput_elements(b, ctx->n);

    ZAP_ITER(b, {
        mat_pow(ctx->B, ctx->A, ctx->p);
...
int main(int argc, char** argv) {
    ...
    zap_compare_group_t* g = zap_compare_group("pow");
