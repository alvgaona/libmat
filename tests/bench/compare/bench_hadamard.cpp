/*
 * bench_hadamard.cpp - Compare HADAMARD: libmat vs Eigen
 *
 * C = A .* B (element-wise multiplication)
 *
 * Build:
 *   make bench-compare-hadamard
 */
...
void bench_libmat(zap_bencher_t* b, void* param) {
    hadamard_ctx_t* ctx = (hadamard_ctx_t*)param;
...
// Eigen: C = A.cwiseProduct(B)
void bench_eigen(zap_bencher_t* b, void* param) {
    hadamard_ctx_t* ctx = (hadamard_ctx_t*)param;
    zap_bencher_set_throughput_bytes(b, 3 * ctx->n * sizeof(Scalar));

    ZAP_ITER(b, {
        ctx->eC->noalias() = ctx->eA->cwiseProduct(*ctx->eB);
...
int main(int argc, char** argv) {
    ...
    zap_compare_group_t* g = zap_compare_group("hadamard");
