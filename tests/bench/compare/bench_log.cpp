/*
 * bench_log.cpp - Compare LOG: libmat vs Eigen
 *
 * B = log(A) (element-wise natural logarithm)
 *
 * Build:
 *   make bench-compare-log
 */
...
static void fill_random_positive(mat_elem_t* data, size_t n) {
    for (size_t i = 0; i < n; i++) {
        data[i] = (Scalar)rand() / RAND_MAX * 100.0f + 0.01f;  // [0.01, 100]
    }
}

// libmat: B = log(A)
void bench_libmat(zap_bencher_t* b, void* param) {
    log_ctx_t* ctx = (log_ctx_t*)param;
...
int main(int argc, char** argv) {
    ...
    zap_compare_group_t* g = zap_compare_group("log");
